...
## v1.7 — Omnichannel Chat Interfaces (✅ Completed)

**Цель:** Оживить каналы связи и дать пользователю возможность напрямую общаться с ассистентом текстом через разные интерфейсы.

**Ключевые фичи:**
- [x] Доработать **Telegram Connector**, чтобы можно было писать боту с телефона (интеграция grammY бота с Event loop / Pipeline).
- [x] Сделать красивый виджет **Chat в Dashboard**, который через WebSocket работает с ядром (отправка сообщений, получение Markdown-ответов).
- [x] Реализовать интерактивный **CLI-чат** в терминале (`jarvis chat`), чтобы можно было переписываться локально.

**Критерии готовности v1.7:**
- Все 3 канала связи (Telegram, Web, CLI) могут отправить сообщение ядру и получить осмысленный ответ. ✅
- LLM Pipeline прозрачно подключен к роутингу входящих сообщений. ✅
